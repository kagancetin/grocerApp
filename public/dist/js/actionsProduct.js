...
let submitAddProduct = (e)=>{
    ...
    if (requiredError.length > 0) {
        ...
        $.post(action,postData,(response)=>{
            ...
            if(response){
                toastr.success(response.name + ' ürünü başarıyla eklenmiştir', 'Başarılı!');
                closeModal("productAddModal");
            }
            else{
                toastr.error('Bilinmeyen bir hata oluştu. Lütfen tekrar deneyiniz', 'Hata!');
                closeModal("productAddModal");
            }
        });
    }
...
let editProductCheck = (el,barcode)=>{
    searchProductByBarcodeOnkeyup(el, result => {
        if (result && result.barcode != barcode) {
            toastr.warning('Ürün zaten '+ result.name +' olarak kayıtlıdır', 'Dikkat!');
